...
class PaginationResultTagSelectView(View):
  def __init__(self, datas: ResultSet[Tag], select_callback: callable, next_callback: callable=None, next_callback_disabled: bool=True, per_page: int=25):
  ...
  def build_select(self):
    ...
    options = [
        discord.SelectOption(
            label=f"{self.datas.index(chapter)+1}. {chapter.get_text(strip=True)}",
            value=chapter.get("href")
        )
  ...
  def next_button(self):
    ...
    has_next_page = self.page < self.max_page
    ...
    button.disabled = not has_next_page
    button.callback = callback
    return button
